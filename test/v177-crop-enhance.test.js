...
test('v1.6.77: HTML 有取消選取 + 完成選取按鈕', () => {
...
test('v1.6.77: WN_cancelSelection 函式存在 (清空 + reset)', () => {
...
test('v1.6.77: WN_completeSelection 函式存在 (顯示教材表單)', () => {
...
test('v1.6.77: Crop box 有 resize handles (左上 + 右下)', () => {
...
test('v1.6.77: Crop box 有 delete (×) + add (+) 按鈕', () => {
...
test('v1.6.77: WN_startResize 函式存在 (左上 + 右下角 resize)', () => {
...
test('v1.6.77: WN_startMove 函式存在 (拖拉移動)', () => {
...
test('v1.6.77: WN_addCropFromCenter 函式存在 (從中心新增)', () => {
...
test('v1.6.77: double click 新增 crop box', () => {
...
test('v1.6.77: Crop box CSS 加強 (藍色 3px + selected 狀態)', () => {
...
test('v1.6.77: title 更新為 [v1.6.77]', () => {
