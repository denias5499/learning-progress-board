...
test('v1.6.67: wrong-notes/index.html 存在', () => {
    assert.ok(wrongNotesExists, 'wrong-notes/index.html 應存在');
});

test('v1.6.67: 附件是獨立 HTML (有 doctype + title)', () => {
...
test('v1.6.67: 附件含 4 個 nav pages', () => {
    if (!wrongNotesExists) return;
    const content = fs.readFileSync(WRONG_NOTES_HTML, 'utf8');
...
test('v1.6.67: 主專案 #stats-wrongnotes 含 iframe 嵌入附件', () => {
...
test('v1.6.67: title 更新為 [v1.6.67]', () => {
    assert.ok(/<title>2026 學習進度看板.*\[v1\.6\.67\]<\/title>/.test(indexHtml));
});

test('v1.6.67: 主專案不再有 wn-import tab', () => {
    assert.ok(!/<div id="wn-import"/.test(indexHtml), '#wn-import 元素應已移除');
});

test('v1.6.67: iframe 高 85vh 寬 100%', () => {
